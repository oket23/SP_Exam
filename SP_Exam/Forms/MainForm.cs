...
public partial class MainForm : Form
{
    ...
    private async void searchWordBtn_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            var searchData = await fileService.SearchWordInFolderAsync(folder, wordTB.Text.Trim(), _cts.Token, progress);
    ...
    private async void classFindBtn_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            var progress = new Progress<Tuple<int, string>>(UpdateProgressBar);
            var folder = GetFolder();
            var searchData = await fileService.FindClassesAndInterfacesAsync(folder, _cts.Token, progress);
    ...
    private void fCaRWBtn_Click(object sender, EventArgs e)
    {
        try
        {
            StartPreset();
